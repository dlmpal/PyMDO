...
def verify_dict_2d(input_vars: List[Variable], output_vars: List[Variable],
    ...
    for out_var in output_vars:
        ...
        for in_var in input_vars:
            ...
            values_dict[out_var.name][in_var.name] = atleast_2d(values_dict[out_var.name][in_var.name]).astype(dtype)
        ...
        if out_var.name in values_dict:
            ...
            for in_var in input_vars:
                if in_var.name in values_dict[out_var.name]:
                    copy[out_var.name][in_var.name] = atleast_2d(values_dict[out_var.name][in_var.name]).copy()
    return copy
...
def dict_to_array_2d(input_vars: List[Variable], output_vars: List[Variable],
    ...
    for out_var in output_vars:
        col_idx = 0
        for in_var in input_vars:
            values_arr[row_idx: row_idx + out_var.size, col_idx: col_idx + in_var.size] = values_dict[out_var.name][in_var.name]
            col_idx += in_var.size
...
def array_to_dict_2d(input_vars: List[Variable], output_vars: List[Variable], values_arr: ndarray) -> Dict[str, Dict[str, ndarray]]:
    ...
    for out_var in output_vars:
        ...
        for in_var in input_vars:
            values_dict[out_var.name][in_var.name] = values_arr[row_idx: row_idx + out_var.size, col_idx: col_idx + in_var.size]
            col_idx += in_var.size
        row_idx += out_var.size
    return values_dict
...
def normalize_dict_2d(input_vars: List[Variable], output_vars: List[Variable], values_dict: Dict[str, Dict[str, ndarray]]):
    for out_var in output_vars:
        for in_var in input_vars:
            values_dict[out_var.name][in_var.name] = in_var.norm_grad(values_dict[out_var.name][in_var.name])
    return values_dict
...
def denormalize_dict_2d(input_vars: List[Variable], output_vars: List[Variable],
    ...
    for out_var in output_vars:
        for in_var in input_vars:
            values_dict[out_var.name][in_var.name] = in_var.denorm_grad(values_dict[out_var.name][in_var.name])
    return values_dict
